    ...
    let
      impl =
        ...
        let
          ...
          matchesAny =
            name: patterns:
            patterns != [ ] && builtins.any (pattern: lib.strings.match pattern name != null) patterns;

          shouldInclude =
            name: type:
            type == "regular" && matchesAny name include && !(matchesAny name exclude) && name != self;

          # Get files from current directory
          currentDirFiles = lib.attrsets.mapAttrsToList (name: _: (lib.path.append path "./${name}")) (
          ...
          shouldRecurse =
            if builtins.isBool recursive then
              recursive
            else if builtins.isInt recursive then
              recursive > 0
            else
              false;
